...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			String toCheck = getStringFromFile(textfile, numToCheck);
			
			long basicStartTime = System.nanoTime();
			for(int i=0; i<trials; i++)
			{
				BasicDocument bd = new BasicDocument(toCheck);
				bd.getFleschScore();
			}
			long basicEndTime = System.nanoTime();
			
			double estTime = (basicEndTime - basicStartTime)/1000000000.0;
			System.out.print(estTime + "\t ");
			
			long efficientStartTime = System.nanoTime();
			for(int i=0; i<trials; i++)
			{
				EfficientDocument ed = new EfficientDocument(toCheck);
				ed.getFleschScore();
			}
			long efficientEndTime = System.nanoTime();
			
			estTime = (efficientEndTime - efficientStartTime)/1000000000.0;
			System.out.print(estTime + "\n");
		}
